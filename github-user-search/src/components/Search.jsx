	...
	return (
		<div className='container mx-auto p-6'>
			<form
				...
				className='bg-[#343a40] p-6 rounded-lg shadow-xl max-w-lg mx-auto'
			>
				<input
					type='text'
			...
			{loading && (
				<p className='text-center mt-4 text-[#f8f9fa]'>Loading...</p>
			)}
			{error && (
			...
			{userData.length > 0 && (
				<div className='mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
					{userData.map(user => (
						<div
							key={user.id}
							className='bg-[#343a40] p-4 rounded-lg shadow-md'
						>
							<img
								src={user.avatar_url}
								alt={user.login}
							...
							/>
							<h3 className='text-center mt-4 text-lg font-semibold text-[#f8f9fa]'>
								{user.login}
							</h3>
							...
							<p className='text-center text-[#6c757d]'>
								Repositories: {user.public_repos || 'No repo'}
							</p>
							<a
								href={user.html_url}
								target='_blank'
